...
class AccountsController(TransactionBase):
	...
	def set_missing_item_details(self, for_validate=False):
		...
		if not hasattr(self, "items"):
			# DH: Added condition above to avoid unnecessary indentation below.
			return
		...
		if self.doctype in ["Quotation", "Sales Order", "Delivery Note", "Sales Invoice", "Daily Order"]:
			...
			parent_dict.update({"document_type": document_type})  # DH: no idea why this matters?
		...
		self.pricing_rules = []
		for item in [ item for item in self.get("items") if item.get("item_code") ]:
			...
			ret = get_item_details(args, doc=self, for_validate=True, overwrite_warehouse=False)
			...
			# 3.  If ret contains a List of pricing rules, apply that logic as well.
